...
def train_one_fold(fold_idx, reader, folds, args):
    params, train_loader, eval_loader = create_dataloader(args.dataset, reader, *folds[fold_idx - 1])
    ...
    if args.synchronized_loading:
        ...
        counts_skel = get_class_counts(train_loader, num_classes, "label_skeleton")
        counts_sens = get_class_counts(train_loader, num_classes, "label_sensor")
        w_skel = make_weights(counts_skel)
        w_sens = make_weights(counts_sens)
        criterion_skel = nn.CrossEntropyLoss(weight=w_skel)
        criterion_sens = nn.CrossEntropyLoss(weight=w_sens)
    ...
    if args.fusion_type == "early":
        ...
        model = ShareLatentModel(**common_kwargs, taskhead_input_dim=params["backbone_dim"] * params["shared_out_channels"]).to(device)
    ...
    def run_epoch(loader, train=True, collect_preds=False):
        ...
        for batch in loader:
            ...
            y_skel = batch["label_skeleton"].to(device)
            y_sensor = batch["label_sensor"].to(device)

            if args.synchronized_loading:
                logits = model(sk, se)
                loss = criterion(logits, y_skel)
            else:
                logits_skel, logits_sens = model(sk, se)
                loss_skel = criterion_skel(logits_skel, y_skel)
                loss_sens = criterion_sens(logits_sens, y_sensor)
                loss = (loss_skel + loss_sens) / 2
            ...
            if args.synchronized_loading:
                correct += (logits.argmax(1) == y_skel).sum().item()
                if collect_preds and not train:
                    trues.extend(y_skel.cpu().tolist())
                    preds.extend(logits.argmax(1).cpu().tolist())
            else:
                correct_sk += (logits_skel.argmax(1) == y_skel).sum().item()
                correct_se += (logits_sens.argmax(1) == y_sensor).sum().item()
                if collect_preds and not train:
                    trues_skel.extend(y_skel.cpu().tolist())
                    preds_skel.extend(logits_skel.argmax(1).cpu().tolist())
                    trues_sens.extend(y_sensor.cpu().tolist())
                    preds_sens.extend(logits_sens.argmax(1).cpu().tolist())
        ...
        if args.synchronized_loading:
            ...
            acc_skel = correct_sk / total * 100
            acc_sens = correct_se / total * 100
            if collect_preds:
    ...
    for epoch in range(params["epochs"]):
        ...
        with torch.no_grad():
            if args.synchronized_loading:
                e_loss, e_acc, trues, preds = run_epoch(eval_loader, train=False, collect_preds=True)
                e_avg = e_acc
            else:
                e_loss, e_acc_skel, e_acc_sens, trues_skel, preds_skel, trues_sens, preds_sens = run_epoch(eval_loader, train=False, collect_preds=True)
                e_avg = (e_acc_skel + e_acc_sens) / 2.0

        # update best metrics
        if e_avg > best_avg_acc:
            best_avg_acc = e_avg
            best_skel_acc = e_acc_skel if not args.synchronized_loading else e_avg
            best_sens_acc = e_acc_sens if not args.synchronized_loading else e_avg
    ...
    if args.synchronized_loading:
        ...
        print(f"\n>>> Fold {fold_idx} Best Mean Acc: {best_avg_acc:.2f}%")
...
for fusion in early late share_latent cheap_xattn; do
  for seed in 42 43 44 2 3 4; do
    nohup python3 baseline/fusion_train.py \
      --fusion_type "$fusion" \
      --seed "$seed" \
    > "logs/async/log_sync_${fusion}_s${seed}.out" 2>&1 &
  done
done
...
for fusion in early late share_latent cheap_xattn; do
  for seed in 42 43 44 2 3 4; do
    nohup python3 baseline/fusion_train.py \
      ...
      --fusion_type "$fusion" \
      --seed "$seed" \
    > "logs/turn/mm_alpha0_1/fusion/sync_no_wl/0527_log_${fusion}_s${seed}.out" 2>&1 &
  done
done
"""