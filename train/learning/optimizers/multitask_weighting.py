...
try:
    import cvxpy as cp
except ImportError:  # optional dependency used only by NashMTL
    cp = None
import numpy as np
import torch
...
class FAMO(WeightMethod):
    ...
    def __init__(
        ...
        self.min_losses = torch.zeros(n_tasks).to(device) # record baseline losses
    ...
    def get_weighted_loss(self, losses, **kwargs):
        ...
        c = (z / D).sum().detach() # normalizer
        loss = (D.log() * z / c).sum()
        return 3*loss, {"weights": z, "logits": self.w.detach().clone()}
...
class NashMTL(WeightMethod):
    ...
    def solve_optimization(self, gtg: np.array):
        if cp is None:
            raise ImportError("cvxpy is required for NashMTL but is not installed")
...
class CAGrad(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for param in shared_parameters:
            grad_dims.append(param.data.numel()) # number of elements (weights) in each parameter
        grads = torch.Tensor(sum(grad_dims), self.n_tasks).to(self.device) # [# of weights.shape, # of tasks]

        for i in range(self.n_tasks):
            if i < self.n_tasks:
                losses[i].backward(retain_graph=True) # computes gradient, set retain_graph to true for next .backward() call
            else:
                losses[i].backward() 
            self.grad2vec(shared_parameters, grads, grad_dims, i) # stores task-specific gradient into vector (grads)
            # multi_task_model.zero_grad_shared_modules()
            for p in shared_parameters:
                p.grad = None

        g, GTG, w_cpu = self.cagrad(grads, alpha=self.c, rescale=1) # skip for now ... 
        self.overwrite_grad(shared_parameters, g, grad_dims)
        return GTG, w_cpu

    def cagrad(self, grads, alpha=0.5, rescale=1):
        """
        grads: gradients for each task. Shape: (num_parameters.shape, num_tasks).

        """
        GG = grads.t().mm(grads).cpu()  # [num_tasks, num_tasks] t: transpose, mm: matrics multiplication. quantifies gradients similarities
        ...
        # solving the constrained optimization problem for stability in multitask learning
    ...
    def grad2vec(shared_params, grads, grad_dims, task):
        # store the .backward() computed task-specific gradient into vector (grads)
