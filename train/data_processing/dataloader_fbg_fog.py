# data_processing/dataloader_fbg_fog.py
        ...
        if self.synchronized:
            ...
            if seed is not None:
                ...
                if self.pose_label_map is not None:
                    # FBG pose keys look like "SUBxx_on_walk_..."; labels map uses "SUBxx_on"
...
def create_fusion_loaders(
    ...
    # ─── Single‐Modality Class‐Balanced Eval (FBG vs FoG) ───────────────
    if modality in ("skeleton", "sensor"):
        # (a) decide which key‐list and which label‐map to use
        if dataset == "fog":
            # For FoG, both skeleton & sensor labels come from subject_label_map
            if modality == "skeleton":
                key_list = eval_ds.pose_ds.keys
                def get_label_from_key(k):
                    return subject_label_map[k.split("_")[0]]
            else:  # sensor‐only
                key_list = eval_ds.sens_ds.keys
                def get_label_from_key(k):
                    return subject_label_map[k.split("_")[0]]
            ...
            if modality == "skeleton":
                key_list = eval_ds.pose_ds.keys
                def get_label_from_key(k):
                    # two‐token prefix, e.g. "SUB01_on"
                    prefix = "_".join(k.split("_")[:2])
                    return pose_label_map[prefix]
            else:  # modality == "sensor"
                key_list = eval_ds.sens_ds.keys
                def get_label_from_key(k):
                    # full key, e.g. "SUB01_on_walk_0"
                    return sensor_label_map[k]

        # (b) group all eval keys by class
        cls2keys = defaultdict(list)
        for k in key_list:
            cls = get_label_from_key(k)
            cls2keys[cls].append(k)

        # (c) compute target = size of largest class
        max_n = max(len(v) for v in cls2keys.values())

        # (d) oversample each class up to max_n
        balanced_keys = []
        for cls, keys_in_cls in cls2keys.items():
            for _ in range(max_n):
                balanced_keys.append(random.choice(keys_in_cls))
        random.shuffle(balanced_keys)

        # (e) replace the eval keys
        if modality == "skeleton":
            eval_ds.pose_ds.keys = balanced_keys
        else:
            eval_ds.sens_ds.keys = balanced_keys

                
    # 4) Class-balanced async eval for both FBG & FoG ─────────────────────────
    if (modality=="multimodal") and not synchronized:
