...
class PDReader():
    """
    Reads processed FBG .npy skeleton data, assigns labels, extracts metadata,
    and structures data for further use by the dataloaders.
    """
    ...
    def __init__(self, joints_path, sensor_path, labels_path):
        ...
        self.metadata_table = self._load_metadata_table()
    ...
    def read_metadata(self, file_name):
        ...
        subject_rows = self.metadata_table[self.metadata_table['ID'] == subject_id]
        return subject_rows.values[:, 1:]

    def _load_metadata_table(self):
        df = self.label_list.copy()
        ...
        for col in ('age', 'height', 'weight', 'bmi'):
            ...
            df[col] = 0.0 if denom == 0 else (df[col] - df[col].min()) / denom
        return df
...
# raw_data = PDReader('data/FBG', 'data/GRF_processed', 'data/PDGinfo.xlsx')
